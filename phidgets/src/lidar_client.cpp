...
void lidarCallback(const sensor_msgs::LaserScanConstPtr& ptr)
{
    ...
    if ((lidar_image != NULL) && (scan.header.frame_id != "")) {
        std::string sensor_index_str = scan.header.frame_id.substr(scan.header.frame_id.length()-1,1);
        int sensor_index = atoi(sensor_index_str.c_str());
        if (sensor_index < no_of_sensors) {
        
            int i,j;
            unsigned char *lidar_image_ = (unsigned char *)lidar_image->imageData;
            if (sensor_index == 0) {
                for (i = lidar_image_width*lidar_image_height*3-1; i >= 0; i--) lidar_image_[i] = 255;
            }

            int increments = (int)scan.ranges.size();
            float ang = (sensor_index * 2 * 3.1415927f / no_of_sensors) + scan.angle_min;
            int cx = lidar_image_width/2;
            int cy = lidar_image_height/2;
            float mult = 1.0f / (lidar_image_dimension_mm * 2 / 1000.0f);
            for (i = 0; i < increments; i++, ang += scan.angle_increment) {
                float range_metres = scan.ranges[i];
                int dx = (int)(cx * range_metres * sin(ang) * mult);
                int dy = (int)(cy * range_metres * cos(ang) * mult);
                int x = cx + dx;
                int y = cy - dy;
    ...
    float cycle_time_sec)
{
    ...
    srv.request.pan_position_left_angle_degrees = pan_position_left_angle_degrees;
    srv.request.pan_position_right_angle_degrees = pan_position_right_angle_degrees;
    srv.request.tilt_position_up = tilt_position_up;
    srv.request.tilt_position_down = tilt_position_down;
    srv.request.tilt_position_up_angle_degrees = tilt_position_up_angle_degrees;
    srv.request.tilt_position_down_angle_degrees = tilt_position_down_angle_degrees;
...
{
    lidar_image = cvCreateImage(cvSize(lidar_image_width, lidar_image_height), 8, 3);
    cvNamedWindow("LIDAR", CV_WINDOW_AUTOSIZE);

    ros::init(argc, argv, "lidar_client");
    ros::NodeHandle n;
    ros::Subscriber lidar_sub = n.subscribe("phidgets/lidar", 1, lidarCallback);
    client_lidar_control = n.serviceClient<phidgets::lidar_control>("phidgets/lidar_control");
    ...
    for (int s = no_of_sensors; s < no_of_sensors + no_of_coupled_sensors; s++) {
    ...
    while(n.ok()) {
        ...
        if (scan_received) {
            ...
            try {                
                listener.lookupTransform("/base_link", "/base_lidar", ros::Time(0), lidar_transform);
            }
