...
//TODO: Reset PTU geändert in rt thever
        Write(" rt "); // reset pan-tilt unit (also clears any bad input on serial port)

        // wait for reset to complete
        int len = 0;
        char temp;
        char response[10] = "!T!T*";
// thever 02.05.2011
        ...
        // thever 02.05.2011
        PMin = -1500; //GetLimit(PTU46_PAN, PTU46_MIN);
        PMax = 1500; //GetLimit(PTU46_PAN, PTU46_MAX);
        // thever 02.05.2011
       TMin = GetLimit(PTU46_TILT, PTU46_MIN);
        TMax = GetLimit(PTU46_TILT, PTU46_MAX);
        PSMin = GetLimit(PTU46_PAN, PTU46_MIN_SPEED);
